...
void ReprocessOculusRawData::rawDataCallback(
    const apl_msgs::RawData::ConstPtr &raw_data) {
  ...
  if ((raw_data->data[0] != liboculus::PacketHeaderLSB) ||
      (raw_data->data[1] != liboculus::PacketHeaderMSB))
    return;

  // n.b. this is a deep copy.  Not sure if it's avoidable
  std::shared_ptr<liboculus::ByteVector> buffer(
      std::make_shared<liboculus::ByteVector>(raw_data->data));
  ...
  if (msgId == messageSimplePingResult) {
    ...
    ROS_INFO_STREAM(std::string(buffer->begin() + sizeof(OculusMessageHeader),
                                buffer->end()));
  }
}
